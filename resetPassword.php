...
<html>
  <head>
    <title>Reset Password for MyKar.in</title>
...
if (!isset($_POST['password'])) {
    $q = "select email,token from users where token='" . $token . "'";
    $r = mysql_query($q);
    ...
    if ($email != '') {
        ...
        echo '<div class="col-md-12">
                                      <center><h3>Invalid link or Password already changed.</h3></center>
                                      <br>
                                      </div>';
        $toShow = 'no';
    }
    ...
    if (!isset($pass)) {
        echo '<form method="post">
                   <br>
                              ...
                              <div class="col-md-6">
                                <input type="submit" id="resetPassword" name="resetPassword" value="Reset Password" class="btn btn-primary form-control" />
                                <br>
                              </div>
                  </form>';
    }
...
if (isset($_POST['password']) && isset($_SESSION['email'])) {
    $passwordUpdateQuery  = "update users set password='" . md5($pass) . "', token='' where email='" . $email . "'";
    $passwordUpdateResult = mysql_query($passwordUpdateQuery);
    if ($passwordUpdateResult) {
        echo '<div class="col-md-12">
                                      <center><h3>Your password is changed successfully. <a href="login.html" style="text-decoration:underline;">Go to login.</a></h3></center>
                                      <br>
                                      </div>';
    } else {
        echo '<div class="col-md-12">
                                      <center><h3>An error occurred</h3></center>
                                      <br>
                                      </div>';
    }
}
